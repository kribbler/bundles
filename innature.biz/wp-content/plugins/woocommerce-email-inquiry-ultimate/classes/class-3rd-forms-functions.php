...
{	
	...
	public static function show_inquiry_form ( $product_id=0, $show_product_info = 0, $open_type = 'full') {
		...
		if (!isset($wc_email_inquiry_settings_custom['contact_form_shortcode'])) $contact_form_shortcode = trim($wc_email_inquiry_3rd_contact_form_settings['contact_form_shortcode']);
		else $contact_form_shortcode = trim(esc_attr($wc_email_inquiry_settings_custom['contact_form_shortcode']));
		
		if ( trim($contact_form_shortcode) == '' ) $contact_form_shortcode = trim($wc_email_inquiry_3rd_contact_form_settings['contact_form_shortcode']);
		...
		$inquiry_form .= '<p>'.wc_ei_ict_t__( 'Custom Form - Contact Form Title', __('You are making an Email Inquiry about:', 'wc_email_inquiry') ).'</p>';
