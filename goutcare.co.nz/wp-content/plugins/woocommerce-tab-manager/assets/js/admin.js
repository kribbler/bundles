jQuery( function($){
	...
	function productTabRowIndexes() {
		$('.woocommerce_product_tabs .woocommerce_product_tab').each(function(index, el){
			$('.product_tab_position', el).val( parseInt( $(el).index('.woocommerce_product_tabs .woocommerce_product_tab') ) );
		});
	};
	...
	$('button.add_product_tab').on('click', function() {
		...
		if (!productTabId) {
			...
			$.post( wc_tab_manager_admin_params.ajax_url, data, function(response) {
				...
				try {
					var qt = quicktags( { id:"producttabcontent" + data.size, buttons:"strong,em,link,block,del,ins,img,ul,ol,li,code,more,spell,close,fullscreen" } );
					// force the quicktag buttons to render by calling the "prive" _buttonsInit() method
					QTags._buttonsInit();
					// hook up the "Upload/Insert" links
					$('#wp-producttabcontent' + data.size + '-wrap').mousedown(function(e){
	...
	$('.woocommerce_product_tabs').on('click', 'button.remove_row', function() {
		var answer = confirm(wc_tab_manager_admin_params.remove_product_tab);
		if (answer) {
