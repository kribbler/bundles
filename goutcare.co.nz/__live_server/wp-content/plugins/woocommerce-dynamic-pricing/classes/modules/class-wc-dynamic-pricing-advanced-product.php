				...
				if ($q >= $rq) {
					...
					switch ($rule['type']) {
						...
						case 'percent_adjustment':
							...
							$adjusted = round(floatval($price) - ( floatval($amount) * $price), (int) get_option('woocommerce_price_num_decimals'));
							$line_total = 0;
							...
							break;
						case 'fixed_price':
							$adjusted = round($amount, (int) get_option('woocommerce_price_num_decimals'));
							$line_total = 0;
							$full_price_quantity = $cart_item['quantity'] - $a;
