...
class WC_Email_Inquiry_Email_Options
{
	...
	public static function panel_page() {
		...
		if (isset($_REQUEST['bt_save_settings'])) {
			...
			if (!isset($customized_settings['inquiry_send_copy'])) $customized_settings['inquiry_send_copy'] = 'no';
		...
		<table class="form-table">
            <tr valign="top">
		    	...
		    	<td class="forminp">                    
                    <input type="text" value="<?php esc_attr_e( stripslashes( $inquiry_email_from_name ) ); ?>" name="<?php echo $option_name; ?>[inquiry_email_from_name]" id="inquiry_email_from_name" style="width:300px;"  />
				</td>
			</tr>
            <tr valign="top">
		    	<th class="titledesc" scope="row"><label for="inquiry_email_from_address"><?php _e( '"From" Email Address', 'wc_email_inquiry' );?></label></th>
		    	<td class="forminp">                    
                    <input type="text" value="<?php esc_attr_e( stripslashes( $inquiry_email_from_address ) ); ?>" name="<?php echo $option_name; ?>[inquiry_email_from_address]" id="inquiry_email_from_address" style="width:300px;"  />
				</td>
			</tr>
            <tr valign="top">
		    	<th class="titledesc" scope="row"><label for="inquiry_send_copy"><?php _e( 'Send Copy to Sender', 'wc_email_inquiry' );?></label></th>
		    	<td class="forminp">
                	<label><input type="checkbox" <?php checked( $inquiry_send_copy, 'yes' ); ?> value="yes" id="inquiry_send_copy" name="<?php echo $option_name; ?>[inquiry_send_copy]" /> <span class=""><?php _e('Checked adds opt in/out option to the bottom of the email inquiry form.', 'wc_email_inquiry');?></span></label>
				</td>
			</tr>
		</table>
