...
function swipe_curl_call($url){
	...
	curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false); // Remove from Live version after fixing SSL certificate issue
	$html = curl_exec($ch);
	
	$has_curl_errors = curl_errno($ch);
	$curl_error="";
...
try{
	...
	if($minRequirementsOK){
		$response =	swipe_curl_call($basicConnectionTestUrl);
		$response_data = json_decode($response, true);
		$basicConnectionTestOK = ( ( $response_data["response_code"] == "400" )  &&  ($response_data["message"] == "Access Denied")  );
	}

	if($basicConnectionTestOK){
		

		$response =  swipe_curl_call($credentialsUrl);
		$response_data = json_decode($response, true);
		$credentialsOK = ($response_data["message"] == "OK" ) ; 
		$merchantStatusOK = $response_data["data"]["mc_account_status"] == "active"  ;
...
#test_config_table th {
	text-align: left;
	background: lightblue;
}
#test_config_table td {
	font-size: 12px;
}
#test_config_table table {
	border-collapse: collapse;
}
#test_config_table tr td {
	padding: 1em 0.5em;
	border: 1px solid #aaa;
}
#test_config_table tr th {
	padding: 1em 0.5em;
	border: 1px solid #aaa;
}
#test_config_table .test-title {
	white-space: nowrap;
	font-weight: bold;
}
#test_config_table .test-result {
	white-space: nowrap;
}
#test_config_table .technical-details {
	color: #888;
	margin: 1em;
	padding: 0.5em;
	border: 1px solid #aaa;
...
<div id="test_config_table">
  <table>
    <thead>
      <tr>
        <th colspan="3"> Swipe Plugin Test </th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td class="test-title"> 1. Minimum Requirements </td>
        <td class="test-result"><?php echo testResultHtml($minRequirementsOK); ?></td>
        <td class="test_info"><?php if($minRequirementsOK === false){ ?>
      ...
      <tr>
        <td class="test-title"> 2. Connection to Swipe </td>
        <td class="test-result"><?php echo testResultHtml($basicConnectionTestOK); ?></td>
        <td class="test_info"><?php if($basicConnectionTestOK === false){ ?>
          ...
          <div class="technical-details"> Technical details: could not connect to: <?php echo htmlentities($basicConnectionTestUrl); ?> </div>
          <?php } ?></td>
      </tr>
      <tr>
        <td class="test-title"> 3. Swipe Merchant Credentials </td>
        <td class="test-result"><?php echo testResultHtml($credentialsOK); ?></td>
        <td class="test_info"><?php if($credentialsOK === false){ ?>
          ...
          <div class="technical-details"> Technical details: called: <?php echo $credentialsUrl; ?> </div>
          <?php } ?></td>
      </tr>
      <tr>
        <td class="test-title"> 4. Swipe Merchant Status </td>
        <td class="test-result"><?php echo testResultHtml($merchantStatusOK); ?></td>
        <td class="test_info"><?php if($merchantStatusOK === false){ ?>
          Your Swipe Merchant account is inactive, or does not have the Payment Page enabled. Please contact Swipe support.
          <div class="technical-details"> Technical details: called: <?php echo $merchantStatusUrl; ?> </div>
          <?php } ?></td>
      </tr>
    </tbody>
  </table>
