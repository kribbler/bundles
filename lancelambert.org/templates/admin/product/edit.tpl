		...
		<form action="/ProductAdmin/Edit/{$product->id}" method="post" enctype="multipart/form-data">
			...
			<div id="tabs">
				...
				<ul class="tabs">
					...
					<li id="tab_custom_text" {if $smarty.post.active_tab=='custom_text'}class="selected"{/if}><a href="#custom_text" onclick="toogleTabs('custom_text'); return false;">Custom text</a></li>
				</ul>
				...
				<div id="general" class="tab" {if !$smarty.post.active_tab || $smarty.post.active_tab=='general'}style="display: block;"{/if}>
					...
					<label>Style:</label>
					<br />
					<input class="textinput" type="text" name="style" value="{$product->style}" maxlength="255" />

					<br />
					<label>Size:</label>
					<br />
					<textarea name="size" class="mceNoEditor" cols="120" rows="1" style="width: 180px; height: 1.33em;">{$product->size}</textarea>
				...
				<div id="data" class="tab"{if $smarty.post.active_tab=='data'} style="display: block;"{/if}>
					...
					*}
					...
					<br />
					<span class="quiet">For infinite quantity leave -1</span>
				...
				<div id="variants" class="tab"{if $smarty.post.active_tab=='variants'} style="display: block;"{/if}>
					...
					<table class="admin_table">
						...
						{foreach from=$product->variants item=variant}
							<tr>
								...
								<td>{$variant->name}</td>
								<td>
									{$smarty.const.CURRENCY_SIGN}<input type="textinput" name="variant_price_change_{$variant->id}" value="{$variant->price_change}" style="width: 50px;" />
								</td>
								<td>
									<input type="textinput" name="variant_quantity_{$variant->id}" value="{$variant->quantity}" style="width: 50px;" />
								</td>
								<td>
									<center><input type="radio" name="variant_{$variant->type}_default" value="{$variant->id}"{if $variant->default} checked="checked"{/if} /></center>
								</td>
							</tr>
						{/foreach}
						</tbody>
					...
					<p>*) For infinite quantity leave -1</p>
				</div>
				...
				<div id="custom_text">
					<fieldset>
						<legend>Custom text #1</legend>
						<table>
							<tr>
								<td>Name</td>
								<td><input name="custom_text_name" type="text" value="{$product->custom_text_name}" /></td>
							</tr>
							<tr>
								<td>Length</td>
								<td><input name="custom_text_length" type="text" value="{$product->custom_text_length}" /></td>
							</tr>
							<tr>
								<td>Description</td>
								<td><textarea name="custom_text_description">{$product->custom_text_description}</textarea> </td>
							</tr>
						</table>
					</fieldset>
					<fieldset>
						...
						<table>
							<tr>
								<td>Name</td>
								<td><input name="custom_text2_name" type="text" value="{$product->custom_text2_name}" /></td>
							</tr>
							<tr>
								<td>Length</td>
								<td><input name="custom_text2_length" type="text" value="{$product->custom_text2_length}" /></td>
							</tr>
							<tr>
								<td>Description</td>
								<td><textarea name="custom_text2_description">{$product->custom_text2_description}</textarea> </td>
							</tr>
						</table>
					</fieldset>
					<fieldset>
						...
						<table>
							<tr>
								<td>Name</td>
								<td><input name="custom_text3_name" type="text" value="{$product->custom_text3_name}" /></td>
							</tr>
							<tr>
								<td>Length</td>
								<td><input name="custom_text3_length" type="text" value="{$product->custom_text3_length}" /></td>
							</tr>
							<tr>
								<td>Description</td>
								<td><textarea name="custom_text3_description">{$product->custom_text3_description}</textarea> </td>
							</tr>
						</table>
					</fieldset>
				</div>
