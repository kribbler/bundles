...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '2.0.2', __( 'WooCommerce Tab Manager', 'woocommerce-tab-manager' ), __FILE__, 'init_woocommerce_tab_manager' );
...
class WC_Tab_Manager extends SV_WC_Plugin {
	...
	public function get_review_url() {
		return 'http://www.skyverge.com/product/woocommerce-tab-manager#review_form';
	}
	...
	/**
	 * Temporary override to support deferred installation
	 *
	 * @since 1.1.1
	...
	protected function do_install() {
		...
		// don't run the installation procedure until the wp_loaded action
		if ( ! $this->wp_loaded_action ) {
			$this->wp_loaded_action = true;

			add_action( 'wp_loaded', array( $this, 'do_install_deferred' ) );
			return;
		}
	...
	/**
	 * Temporary helper method to defer installation, until supported by the
	 * plugin framework.
	 *
	 * Delegates to `do_install()`
	 *
	 * @since 1.1.1
	 */
	public function do_install_deferred() {
		$this->do_install();
	}
