...
function wc_tab_manager_admin_enqueue_scripts() {
	...
	if ( 'product' == $screen->id || ( isset( $_REQUEST['page'] ) && 'tab_manager' == $_REQUEST['page'] ) ) {
		wp_enqueue_script( 'wc_tab_manager_admin', $wc_tab_manager->get_plugin_url() . '/assets/js/admin.js', array( 'jquery' ) );
	...
	if ( isset( $_REQUEST['page'] ) && 'tab_manager' == $_REQUEST['page'] ) {
		...
		if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_2_1() ) {
			...
			wp_enqueue_script( 'woocommerce_admin_meta_boxes', WC()->plugin_url() . '/assets/js/admin/meta-boxes.js', array( 'jquery', 'jquery-ui-datepicker', 'jquery-ui-sortable', 'accounting', 'round' ), WC()->version );
			...
			wp_localize_script( 'woocommerce_writepanel', 'woocommerce_writepanel_params', $params );
		}
