		...
		<div class="content">
			<div class="post_content<?php if($portfolio_post_a_type){?> cre-animate<?php }?>"<?php if($portfolio_post_a_type){ echo $animation_att_pp;}?>>
				...
				<?php $alt = get_post_meta($post_thumbnail_id, '_wp_attachment_image_alt', true);?>
					...
					<img style="max-width: 700px" class=" alignright page-thumbnail img-responsive<?php if($portfolio_thumbnails_a_type){?> cre-animate<?php }?>" alt="<?php echo $alt; ?>" src="<?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'portfolio-page' ); echo $image[0];?>"<?php if($portfolio_thumbnails_a_type){ echo $animation_att;}?> />
				<?php the_content(); ?>
			</div>
			<?php if($details){?>
