...
function smooth_post_processor_default( $posts, $smooth_slider,$out_echo){
	...
	foreach($posts as $post) {
		...
		$slide_fancy_url = get_post_meta($post_id, 'portfolio_options_FancyBox', true);
		//var_dump($slide_fancy_url);
		//portfolio_options_FancyBox
		//filter hook
...
//2.1 changes end	
		...
		$img_args = array(
			...
			'height' => false,
			'echo' => false,
			'permalink' => ($slide_fancy_url) ? $slide_fancy_url : $permalink,
			'style'=> $smooth_slider_css['smooth_slider_thumbnail'],
			'class_fancy' => ($slide_fancy_url) ? 'fancybox-youtube' : ''
		);

		$smooth_slide_image=smooth_sslider_get_the_image($img_args);
		//var_dump($smooth_slide_image);
		...
		/* Added for embeding any shortcode on slide - end */
			...
			//var_dump($content_limit); var_dump($content_chars);
		...
		if ($smooth_slider['image_only'] == '1') { 
		   ...
		   if($permalink!='') {
			...
			$html .= '<span class="d_slider_content" '.$smooth_slider_css['smooth_slider_span'].'> '.$slider_excerpt.'</span>
