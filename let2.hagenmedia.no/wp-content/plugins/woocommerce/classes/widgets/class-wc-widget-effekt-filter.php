<?php
/**
 * Effekt Filter Widget and related functions
 *
 * Generates a range slider to filter products by effekt.
 *
 * @author 		WooThemes
 * @category 	Widgets
...
class WC_Widget_Effekt_Filter extends WP_Widget {
	 ...
	 * @return void
	 */
	function WC_Widget_Effekt_Filter() {

		/* Widget variable settings. */
		$this->woo_widget_cssclass = 'woocommerce widget_effekt_filter';
		$this->woo_widget_description = __( 'Shows a effekt filter slider in a widget which lets you narrow down the list of shown products when viewing product categories.', 'woocommerce' );
		$this->woo_widget_idbase = 'woocommerce_effekt_filter';
		$this->woo_widget_name = __( 'WooCommerce Effekt Filter', 'woocommerce' );

		/* Widget settings. */
		$widget_ops = array( 'classname' => $this->woo_widget_cssclass, 'description' => $this->woo_widget_description );

		/* Create the widget. */
		$this->WP_Widget('effekt_filter', $this->woo_widget_name, $widget_ops);
	}
	...
	function widget_original( $args, $instance ) {
		...
		$min_effekt = isset( $_GET['min_effekt'] ) ? esc_attr( $_GET['min_effekt'] ) : '';
		$max_effekt = isset( $_GET['max_effekt'] ) ? esc_attr( $_GET['max_effekt'] ) : '';

		wp_enqueue_script( 'wc-effekt-slider' );

		wp_localize_script( 'wc-effekt-slider', 'woocommerce_effekt_slider_params', array(
			'currency_symbol' 	=> get_woocommerce_currency_symbol(),
			'currency_pos'      => get_option( 'woocommerce_currency_pos' ),
			'min_effekt'			=> $min_effekt,
			'max_effekt'			=> $max_effekt
		) );
		...
		if ( sizeof( $woocommerce->query->layered_nav_product_ids ) === 0 ) {
			$max = ceil( $wpdb->get_var(
				$wpdb->prepare('
					...
					WHERE meta_key = \'%3$s\'
				', $wpdb->posts, $wpdb->postmeta, 'effekt' )
			) );
		} else {
			$max = ceil( $wpdb->get_var(
				$wpdb->prepare('
				...
				', $wpdb->posts, $wpdb->postmeta, 'effekt', implode( ',', $woocommerce->query->layered_nav_product_ids )
			) ) );
		}
		...
		echo '<form method="get" action="' . $form_action . '">
			<div class="effekt_slider_wrapper">
				<div class="effekt_slider" style="display:none;"></div>
				<div class="effekt_slider_amount">
					<input type="text" id="min_effekt" name="min_effekt" value="'.esc_attr( $min_effekt ).'" data-min="'.esc_attr( $min ).'" placeholder="'.__('Min effekt', 'woocommerce' ).'" />
					<input type="text" id="max_effekt" name="max_effekt" value="'.esc_attr( $max_effekt ).'" data-max="'.esc_attr( $max ).'" placeholder="'.__( 'Max effekt', 'woocommerce' ).'" />
					<button type="submit" class="button">'.__( 'Filter', 'woocommerce' ).'</button>
					<div class="effekt_label" style="display:none;">
						'.__( 'Effekt:', 'woocommerce' ).' <span class="from"></span> &mdash; <span class="to"></span>
					</div>
					'.$fields.'
					<div class="clear"></div>
				</div>
...
public function widget( $args, $instance ) {
		...
		if ( sizeof( $woocommerce->query->layered_nav_product_ids ) === 0 ) {
			$min = floor( $wpdb->get_var(
				$wpdb->prepare('
					...
					AND meta_value != ""
				', $wpdb->posts, $wpdb->postmeta, 'effekt', '_min_variation_effekt' )
			) );
			$max = ceil( $wpdb->get_var(
				$wpdb->prepare('
					SELECT max(meta_value + 0)
		...
		$effekt_range_01 = $instance['my_yr_a1_1'] . ' - ' . $instance['my_yr_a1_2'];
		$effekt_range_02 = $instance['my_yr_a2_1'] . ' - ' . $instance['my_yr_a2_2'];
		$effekt_range_03 = $instance['my_yr_a3_1'] . ' - ' . $instance['my_yr_a3_2'];
		$effekt_range_04 = $instance['my_yr_a4_1'] . ' - ' . $instance['my_yr_a4_2'];
		$effekt_range_05 = $instance['my_yr_a5_1'] . ' - ' . $instance['my_yr_a5_2'];
		$effekt_range_06 = $instance['my_yr_a6_1'] . ' - ' . $most_valuable;
		
		$total_effekt_range_00 = $this->get_total_items();
		$total_effekt_range_01 = $this->get_total_items($instance['my_yr_a1_1'], $instance['my_yr_a1_2']);
		$total_effekt_range_02 = $this->get_total_items($instance['my_yr_a2_1'], $instance['my_yr_a2_2']);
		$total_effekt_range_03 = $this->get_total_items($instance['my_yr_a3_1'], $instance['my_yr_a3_2']);
		$total_effekt_range_04 = $this->get_total_items($instance['my_yr_a4_1'], $instance['my_yr_a4_2']);
		$total_effekt_range_05 = $this->get_total_items($instance['my_yr_a5_1'], $instance['my_yr_a5_2']);
		$total_effekt_range_06 = $this->get_total_items($instance['my_yr_a6_1'], $most_valuable);
		
		?>
		
		
		<ul class="effekt_range_amount">
			<?php if ($total_effekt_range_00){?>
				<li>
					<a href="<?php echo $form_action?>"><?php _e('Alle', 'woocommmerce')?></a>
					<small class="count"><?php echo $total_effekt_range_00?></small>
				</li>
			<?php }?>
			
			<?php if ($total_effekt_range_01){?>
				<?php if ($_GET['min_effekt'] . ' - ' . $_GET['max_effekt'] == $effekt_range_01){?>
				<li class="chosen_effekt">
					<a href="<?php echo $form_action?>"><?php echo $effekt_range_01?></a>
					<small class="count"><?php echo $total_effekt_range_01?></small>
				</li>
				<?php } else {?>
				<li>
					<a href="?min_effekt=<?php echo $instance['my_yr_a1_1']?>&max_effekt=<?php echo $instance['my_yr_a1_2']?>"><?php echo $effekt_range_01?></a>
					<small class="count"><?php echo $total_effekt_range_01?></small>
				</li>
				<?php }?>
			<?php }?>
			
			<?php if ($total_effekt_range_02){?>
				<?php if ($_GET['min_effekt'] . ' - ' . $_GET['max_effekt'] == $effekt_range_02){?>
				<li class="chosen_effekt">
					<a href="<?php echo $form_action?>"><?php echo $effekt_range_02?></a>
					<small class="count"><?php echo $total_effekt_range_02?></small>
				</li>
				<?php } else {?>
				<li>
					<a href="?min_effekt=<?php echo $instance['my_yr_a2_1']?>&max_effekt=<?php echo $instance['my_yr_a2_2']?>"><?php echo $effekt_range_02?></a>
					<small class="count"><?php echo $total_effekt_range_02?></small>
				</li>
				<?php }?>
			<?php }?>
			
			<?php if ($total_effekt_range_03){?>
				<?php if ($_GET['min_effekt'] . ' - ' . $_GET['max_effekt'] == $effekt_range_03){?>
				<li class="chosen_effekt">
					<a href="<?php echo $form_action?>"><?php echo $effekt_range_03?></a>
					<small class="count"><?php echo $total_effekt_range_03?></small>
				</li>
				<?php } else {?>
				<li>
					<a href="?min_effekt=<?php echo $instance['my_yr_a3_1']?>&max_effekt=<?php echo $instance['my_yr_a3_2']?>"><?php echo $effekt_range_03?></a>
					<small class="count"><?php echo $total_effekt_range_03?></small>
				</li>
				<?php }?>
			<?php }?>
			
			<?php if ($total_effekt_range_04){?>
				<?php if ($_GET['min_effekt'] . ' - ' . $_GET['max_effekt'] == $effekt_range_04){?>
				<li class="chosen_effekt">
					<a href="<?php echo $form_action?>"><?php echo $effekt_range_04?></a>
					<small class="count"><?php echo $total_effekt_range_04?></small>
				</li>
				<?php } else {?>
				<li>
					<a href="?min_effekt=<?php echo $instance['my_yr_a4_1']?>&max_effekt=<?php echo $instance['my_yr_a4_2']?>"><?php echo $effekt_range_04?></a>
					<small class="count"><?php echo $total_effekt_range_04?></small>
				</li>
				<?php }?>
			<?php }?>
			
			<?php if ($total_effekt_range_05){?>
				<?php if ($_GET['min_effekt'] . ' - ' . $_GET['max_effekt'] == $effekt_range_05){?>
				<li class="chosen_effekt">
					<a href="<?php echo $form_action?>"><?php echo $effekt_range_05?></a>
					<small class="count"><?php echo $total_effekt_range_05?></small>
				</li>
				<?php } else {?>
				<li>
					<a href="?min_effekt=<?php echo $instance['my_yr_a5_1']?>&max_effekt=<?php echo $instance['my_yr_a5_2']?>"><?php echo $effekt_range_05?></a>
					<small class="count"><?php echo $total_effekt_range_05?></small>
				</li>
				<?php }?>
			<?php }?>
			
			<?php if ($total_effekt_range_06){?>
				<?php if ($_GET['min_effekt'] . ' - ' . $_GET['max_effekt'] == $effekt_range_06){?>
				<li class="chosen_effekt">
					<a href="<?php echo $form_action?>"><?php echo $effekt_range_06?></a>
					<small class="count"><?php echo $total_effekt_range_06?></small>
				</li>
				<?php } else {?>
				<li>
					<a href="?min_effekt=<?php echo $instance['my_yr_a6_1']?>&max_effekt=<?php echo $instance['my_yr_a6_2']?>"><?php echo $effekt_range_06?></a>
					<small class="count"><?php echo $total_effekt_range_06?></small>
				</li>
				<?php }?>
			<?php }?>
		...
		echo '<form method="get" action="' . esc_attr( $form_action ) . '">
			<div class="effekt_slider_wrapper" style="display: none">
				<div class="effekt_slider" style="display:none;"></div>
				<div class="effekt_slider_amount">
					<input type="text" id="min_effekt" name="min_effekt" value="' . esc_attr( $min_effekt ) . '" data-min="'.esc_attr( $min ).'" placeholder="'.__('Min effekt', 'woocommerce' ).'" />
					<input type="text" id="max_effekt" name="max_effekt" value="' . esc_attr( $max_effekt ) . '" data-max="'.esc_attr( $max ).'" placeholder="'.__( 'Max effekt', 'woocommerce' ).'" />
		...
		console.log("active toooooo");
				$("input[name=effekt_range]").change(function(){
					var effekt_range = $(this).val();
					effekt_range = effekt_range.split(" - ");
					$("#min_effekt").val(effekt_range[0]);
					$("#max_effekt").val(effekt_range[1]);
				});

				$("#effekt_range_form").submit(function(){
					var max_effekt = $("#max_effekt").val();
					if (!max_effekt){
						$("#max_effekt").remove();
					}
				});
	...
	function get_total_items($min = NULL, $max = NULL){
		...
		$effekt_range_query = NULL;
		if (isset($min) && isset($max)){
			$effekt_range_query = 'AND meta_value >= '.$min.' AND meta_value <= '.$max;
		}
...
//echo "<pre>";var_dump($_chosen_attributes);
	...
	function update_original( $new_instance, $old_instance ) {
		if (!isset($new_instance['title']) || empty($new_instance['title'])) $new_instance['title'] = __( 'Filter by effekt', 'woocommerce' );
