...
class WC_Widget_Antalseter_Filter extends WP_Widget {
	 ...
	 * @return void
	 */
	function WC_Widget_Antalseter_Filter() {

		/* Widget variable settings. */
		$this->woo_widget_cssclass = 'woocommerce widget_antalseter_filter';
		$this->woo_widget_description = __( 'Shows a antalseter filter slider in a widget which lets you narrow down the list of shown products when viewing product categories.', 'woocommerce' );
		$this->woo_widget_idbase = 'woocommerce_antalseter_filter';
		$this->woo_widget_name = __( 'WooCommerce Antalseter Filter', 'woocommerce' );

		/* Widget settings. */
		$widget_ops = array( 'classname' => $this->woo_widget_cssclass, 'description' => $this->woo_widget_description );

		/* Create the widget. */
		$this->WP_Widget('antalseter_filter', $this->woo_widget_name, $widget_ops);
	}
	...
	function widget_original( $args, $instance ) {
		...
		wp_localize_script( 'wc-antalseter-slider', 'woocommerce_antalseter_slider_params', array(
			...
			'min_antalseter'			=> $min_antalseter,
			'max_antalseter'			=> $max_antalseter
		) );
		...
		if ( sizeof( $woocommerce->query->layered_nav_product_ids ) === 0 ) {
			...
			$max = ceil( $wpdb->get_var(
				...
				', $wpdb->posts, $wpdb->postmeta, 'antalseter', implode( ',', $woocommerce->query->layered_nav_product_ids )
			) ) );
		}
		...
		echo '<form method="get" action="' . $form_action . '">
			<div class="antalseter_slider_wrapper">
...
//echo "<pre>";var_dump($_chosen_attributes);
	...
	function update_original( $new_instance, $old_instance ) {
		if (!isset($new_instance['title']) || empty($new_instance['title'])) $new_instance['title'] = __( 'Filter by antalseter', 'woocommerce' );
