		...
		if ( count( $terms ) > 0 ) {
			...
			if ( $display_type == 'dropdown' ) {
				...
				if ( $current_tax && $taxonomy == $current_tax ) {
					...
					$found = false;
var_dump(wc_attribute_label( $taxonomy ));
				...
				foreach ( $terms as $term ) {
					...
					if ( isset( $_GET['min_kilometer'] ) )
						$link = add_query_arg( 'min_kilometer', $_GET['min_kilometer'], $link );

					if ( isset( $_GET['max_kilometer'] ) )
						$link = add_query_arg( 'max_kilometer', $_GET['max_kilometer'], $link );
					
					if ( isset( $_GET['min_year'] ) )
						$link = add_query_arg( 'min_year', $_GET['min_year'], $link );

					if ( isset( $_GET['max_year'] ) )
						$link = add_query_arg( 'max_year', $_GET['max_year'], $link );
						
					if ( isset( $_GET['min_antalseter'] ) )
						$link = add_query_arg( 'min_antalseter', $_GET['min_antalseter'], $link );

					if ( isset( $_GET['max_antalseter'] ) )
						$link = add_query_arg( 'max_antalseter', $_GET['max_antalseter'], $link );
					
					if ( isset( $_GET['min_effekt'] ) )
						$link = add_query_arg( 'min_effekt', $_GET['min_effekt'], $link );

					if ( isset( $_GET['max_effekt'] ) )
						$link = add_query_arg( 'max_effekt', $_GET['max_effekt'], $link );
