...
class WC_Query {
	...
	/**
	 * Kilometer filter Init
	 */
	public function kilometer_filter_init() {
		if ( is_active_widget( false, false, 'woocommerce_kilometer_filter', true ) && ! is_admin() ) {

			$suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';

			wp_register_script( 'wc-kilometer-slider', WC()->plugin_url() . '/assets/js/frontend/kilometer-slider' . $suffix . '.js', array( 'jquery-ui-slider' ), WC_VERSION, true );

			wp_localize_script( 'wc-kilometer-slider', 'woocommerce_kilometer_slider_params', array(
				'currency_symbol' 	=> get_woocommerce_currency_symbol(),
				'currency_pos'      => get_option( 'woocommerce_currency_pos' ),
			...
			add_filter( 'loop_shop_post_in', array( $this, 'kilometer_filter' ) );
		}
	}

	/**
	 * Kilometer Filter post filter
	 *
	 * @param array $filtered_posts
	 * @return array
	 */
	public function kilometer_filter( $filtered_posts ) {
	    global $wpdb;

	    if ( isset( $_GET['max_kilometer'] ) && isset( $_GET['min_kilometer'] ) ) {

	        $matched_products = array();
	        $min 	= floatval( $_GET['min_kilometer'] );
	        $max 	= floatval( $_GET['max_kilometer'] );

	        $matched_products_query = apply_filters( 'woocommerce_kilometer_filter_results', $wpdb->get_results( $wpdb->prepare("
	        	SELECT DISTINCT ID, post_parent, post_type FROM $wpdb->posts
				INNER JOIN $wpdb->postmeta ON ID = post_id
				WHERE post_type IN ( 'product', 'product_variation' ) AND post_status = 'publish' AND meta_key = %s AND meta_value BETWEEN %d AND %d
			", 'kilometer', $min, $max ), OBJECT_K ), $min, $max );
	 ...
	 * Year filter Init
	 */
	public function year_filter_init() {
		if ( is_active_widget( false, false, 'woocommerce_year_filter', true ) && ! is_admin() ) {

			$suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';

			wp_register_script( 'wc-year-slider', WC()->plugin_url() . '/assets/js/frontend/year-slider' . $suffix . '.js', array( 'jquery-ui-slider' ), WC_VERSION, true );
			...
			add_filter( 'loop_shop_post_in', array( $this, 'year_filter' ) );
		}
	}

	/**
	 * Year Filter post filter
	 *
	 * @param array $filtered_posts
	 * @return array
	 */
	public function year_filter( $filtered_posts ) {
	    global $wpdb;

	    if ( isset( $_GET['max_year'] ) && isset( $_GET['min_year'] ) ) {

	        $matched_products = array();
	        $min 	= floatval( $_GET['min_year'] );
	        $max 	= floatval( $_GET['max_year'] );

	        $matched_products_query = apply_filters( 'woocommerce_year_filter_results', $wpdb->get_results( $wpdb->prepare("
	        	SELECT DISTINCT ID, post_parent, post_type FROM $wpdb->posts
				INNER JOIN $wpdb->postmeta ON ID = post_id
				WHERE post_type IN ( 'product', 'product_variation' ) AND post_status = 'publish' AND meta_key = %s AND meta_value BETWEEN %d AND %d
			", 'year', $min, $max ), OBJECT_K ), $min, $max );
	...
	/**
	 * Antalseter filter Init
	 */
	public function antalseter_filter_init() {
		if ( is_active_widget( false, false, 'woocommerce_antalseter_filter', true ) && ! is_admin() ) {

			$suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';

			wp_register_script( 'wc-antalseter-slider', WC()->plugin_url() . '/assets/js/frontend/antalseter-slider' . $suffix . '.js', array( 'jquery-ui-slider' ), WC_VERSION, true );

			wp_localize_script( 'wc-antalseter-slider', 'woocommerce_antalseter_slider_params', array(
				'currency_symbol' 	=> get_woocommerce_currency_symbol(),
				'currency_pos'      => get_option( 'woocommerce_currency_pos' ),
			...
			add_filter( 'loop_shop_post_in', array( $this, 'antalseter_filter' ) );
		}
	}

	/**
	 * Antalseter Filter post filter
	 *
	 * @param array $filtered_posts
	 * @return array
	 */
	public function antalseter_filter( $filtered_posts ) {
	    global $wpdb;

	    if ( isset( $_GET['max_antalseter'] ) && isset( $_GET['min_antalseter'] ) ) {

	        $matched_products = array();
	        $min 	= floatval( $_GET['min_antalseter'] );
	        $max 	= floatval( $_GET['max_antalseter'] );

	        $matched_products_query = apply_filters( 'woocommerce_antalseter_filter_results', $wpdb->get_results( $wpdb->prepare("
	        	SELECT DISTINCT ID, post_parent, post_type FROM $wpdb->posts
				INNER JOIN $wpdb->postmeta ON ID = post_id
				WHERE post_type IN ( 'product', 'product_variation' ) AND post_status = 'publish' AND meta_key = %s AND meta_value BETWEEN %d AND %d
			", 'antalseter', $min, $max ), OBJECT_K ), $min, $max );
	 ...
	 * Effekt filter Init
	 */
	public function effekt_filter_init() {
		if ( is_active_widget( false, false, 'woocommerce_effekt_filter', true ) && ! is_admin() ) {

			$suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';

			wp_register_script( 'wc-effekt-slider', WC()->plugin_url() . '/assets/js/frontend/effekt-slider' . $suffix . '.js', array( 'jquery-ui-slider' ), WC_VERSION, true );
			...
			add_filter( 'loop_shop_post_in', array( $this, 'effekt_filter' ) );
		}
	}

	/**
	 * Effekt Filter post filter
	 *
	 * @param array $filtered_posts
	 * @return array
	 */
	public function effekt_filter( $filtered_posts ) {
	    global $wpdb;

	    if ( isset( $_GET['max_effekt'] ) && isset( $_GET['min_effekt'] ) ) {

	        $matched_products = array();
	        $min 	= floatval( $_GET['min_effekt'] );
	        $max 	= floatval( $_GET['max_effekt'] );

	        $matched_products_query = apply_filters( 'woocommerce_effekt_filter_results', $wpdb->get_results( $wpdb->prepare("
	        	SELECT DISTINCT ID, post_parent, post_type FROM $wpdb->posts
				INNER JOIN $wpdb->postmeta ON ID = post_id
				WHERE post_type IN ( 'product', 'product_variation' ) AND post_status = 'publish' AND meta_key = %s AND meta_value BETWEEN %d AND %d
			", 'effekt', $min, $max ), OBJECT_K ), $min, $max );
