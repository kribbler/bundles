...
class WPUF_Dashboard {
    ...
    function post_listing( $post_type ) {
		...
		<div class="container_12">
			<div class="grid_8">
		        <h2 class="page-head" style="display: none">
		            ...
		            <table class="wpuf-table" cellpadding="0" cellspacing="0">
		                <thead>
		                    <tr>
		                        ...
		                        ?>
		                        <th style="width: 50% !important"><?php _e( 'Title', 'wpuf' ); ?></th>
		                        <th style="width: 10% !important"><?php _e( 'Status', 'wpuf' ); ?></th>
		                        <?php
		                        if ( 'yes' == $charging_enabled ) {
		                            echo '<th>' . __( 'Payment', 'wpuf' ) . '</th>';
		                        }
		                        ?>
		                        <th style="width: 30% !important"><?php _e( 'Options', 'wpuf' ); ?></th>
		                    </tr>
		                </thead>
		                <tbody>
		                    <?php
		                        ...
		                        <tr>
		                            ...
		                            <td>
		                                <?php if ( wpuf_get_option( 'enable_post_edit', 'wpuf_others', 'yes' ) == 'yes' ) { ?>
		                                    ...
		                                    ?>
		                                    <a class="i_am_purple" href="<?php echo wp_nonce_url( $url . '?pid=' . $post->ID, 'wpuf_edit' ); ?>"><?php _e( 'Edit', 'wpuf' ); ?></a>
		                                <?php } else { ?>
		                                    &nbsp;
		                                <?php } ?>
		
		                                <?php if ( wpuf_get_option( 'enable_post_del', 'wpuf_others', 'yes' ) == 'yes' ) { ?>
		                                    <a class="i_am_purple" href="<?php echo wp_nonce_url( "?action=del&pid=" . $post->ID, 'wpuf_del' ) ?>" onclick="return confirm('Are you sure to delete this post?');"><?php _e( 'Delete', 'wpuf' ); ?></a>
		                                <?php } ?>
		                                <?php if ( get_post_status( $post->ID ) == 'publish' ) { ?>
		                                	<a class="i_am_purple" target="_blank" href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'wpuf' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php _e( 'View Live Ad', 'wpuf' ); ?></a>
		                                <?php } ?>
		                            </td>
		                        </tr>
		                    <?php } ?>
