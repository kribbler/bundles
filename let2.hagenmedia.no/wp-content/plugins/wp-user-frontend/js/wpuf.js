    ...
    var WPUF_Obj = {
        init: function () {
			...
			$('#get_car_info').click(function(){
        		var reg_nr = $('#new-post-registreringsnummer').val();
        		console.log('hey! I am here!' + reg_nr);
        		var el = $(this),
                    data = {
                        'attach_id' : el.data('id'),
                        'nonce' : wpuf.nonce,
                        'action' : 'wpuf_get_car_info',
                        'reg_nr' : reg_nr
                    };
				console.log(data);
                $.post(wpuf.ajaxurl, data, function(response_text){
                	console.log(response_text);
                    var response = jQuery.parseJSON(response_text);
					console.log(response);
					
					if ($.isEmptyObject(response)){
						$('#car_info_fetch').addClass('message_red');
						$('#car_info_fetch').slideDown('fast');
						$('#car_info_fetch').html('Bilen ble ikke funnet. Forsøk gjerne igjen.');
					} else {
						$('#car_info_fetch').addClass('message_green');
						$('#car_info_fetch').slideDown('fast');
						$('#car_info_fetch').html('Bilen ble funnet. Detaljer er forhåndsutfyllt i skjemaet under.');
					}
					if (response.new_post_avgiftsklasse)	$('#new-post-avgiftsklasse').val(response.new_post_avgiftsklasse);
					if (response.new_post_1_gang_reg)		$('#new-post-1-gang-reg').val(response.new_post_1_gang_reg);
					if (response.antall_seter)				$('#new-post-antall-seter').val(response.antall_seter);
					if (response.farge)						$('#new-post-farge').val(response.farge);
					if (response.drivstoff)					$('#new-post-drivstoff').val(response.drivstoff);
					if (response.one_gang_reg)				$('#new-post-1-gang-reg').val(response.one_gang_reg);
					if (response.new_post_sylindervolum)	$('#new-post-sylindervolum').val(response.new_post_sylindervolum);
					
					if (response.effekt)					$('#new-post-effekt').val(response.effekt);
                    return false;
                });
        	});
