...
$brand_selected = true;
		...
		if ($title == "Modell"){
			//echo "<pre>";var_dump($_chosen_attributes);echo "</pre>";
        ...
        if ($display_type=='dropdown') {
            ...
            if ( $current_tax && $taxonomy == $current_tax ) {
				...
				if ($woocommerce->attribute_label( $taxonomy ) == 'Merke'){
					echo '<option value="'.$link.'">'. sprintf( __('Alle', 'woocommerce') ) .'</option>';
				} else {
                ...
                foreach ($terms as $term) {
					...
					if ($title == "Merke"){
						if (!$selected) {
							if (isset($link_terms['pa_modell'])){
								$link = str_replace("/" . array_shift($link_terms['pa_modell']), "", $link);
							}
							//echo "<pre>";var_dump($link);
							//var_dump(array_shift($link_terms['pa_modell']));
							//echo "</pre>";
						}
					}
