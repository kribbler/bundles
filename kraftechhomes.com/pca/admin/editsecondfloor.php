...
<?php
	...
	if(isset($_POST['btnSubmit'])&& $_POST['btnSubmit']=="edit home"){		
			...
			if(isset($_POST['virtualtour']) && $_POST['virtualtour']=='File'){ 
				...
				if(isset($_FILES['filevirtualTour']['name']) && $_FILES['filevirtualTour']['name']!=''){
					...
					$filevirtualTour = time().'_'.strtolower($filevirtualTour);
 					...
 					move_uploaded_file($_FILES['filevirtualTour']['tmp_name'],"videos/SecondFloorMasters/". $filevirtualTour);
		   ...
		   if(isset($_POST['virtualtour']) && $_POST['virtualtour']=='Link'){ 
			...
			if($filevirtualTour!=''){
				...
				$query		=	"update tbl_secondfloormasters set
				...
				foreach($_FILES['imageFiles']['name'] as $filename){
				 ...
				 if(!empty($_FILES['imageFiles']['name'])){
					...
					if($filename!=''){											
					   ...
					   move_uploaded_file($_FILES['imageFiles']['tmp_name'][$i],"image/SecondFloorMasters/". $filename);

					   ///////////////////

							$large_image_location = "image/SecondFloorMasters/". $filename ; 
   ...
   $inquery="insert into   tbl_secondfloormasters_img	set	pretableId	=	'".$insertId."', imgname					=	'".$filename."', imagetext='".$imagetext."'";
...
$(document).ready(function () {        
        ...
        $("table#myTable tr:last").after('<tr id=\"myRow'+counter+'\"> 		<td> Image'+counter+' </td>	<td> 	<input type="text" name="imageFilesText[]" id="imageFilesText" />			<td><input type="file" name="imageFiles[]" id="imageFiles" /></td>		<td><a href="#." onClick="test('+counter+');"> Remove </a></td>   </tr>');
...
<div class="left-col">
 ...
 <h2> Edit - Second Floor Masters</h2>
...
<table cellpadding="3" width="400" style="margin-left:150px;">
    ...
    <tr>
        ...
        <td>
        	...
        	<div><input style="min-width:250px;" type="text" name="txtExtratext" value="<?php echo isset($_POST['txtExtratext'])?$_POST['txtExtratext']:$fetchdata['extratext']; ?>" /></div>
...
<td colspan="3">
  ...
  <table id="myTable"  style="margin-left:0px">
                     	...
                     	<p style="padding-left:150px;"><a href="updatesecondfloor.php?updateimage=<?php echo $_GET['val'].'&id='.$editid; ?>">Edit old images</a></p>
