...
foreach ( $module_meta['gMediaQuery'] as $i => $tab ) {
	...
	foreach ( $gMediaQuery as $item ) {
		...
		$a[]   = "	{'id':'{$item->ID}','image': '/{$gmOptions['folder']['image']}/{$item->gmuid}','thumb': '/{$gmOptions['folder']['link']}/{$thumb['file']}','title': " . json_encode( strip_tags($item->title) ) . ",'text': " .  json_encode(wpautop($item->description)) . ",'media': '','link': " . json_encode($meta['link']) . ",'linkTarget': ''}";
	}
}
if ( empty( $a ) ) {
	$continue = true;
