	...
	public function __construct( $response, $delimiter, $encap_char, $custom_fields )
	{
		if ( $response ) {
			...
			if ( count( $this->_response_array ) < 10 ) {
				$this->approved = false;
				$this->error = true;
				$this->error_message = sprintf( __('Unrecognized response from AuthorizeNet: %s', 'dti-authorize-pro'), $response );
				return;
			}
			...
			$this->approved = false;
			$this->error = true;
			$this->error_message = __('Error connecting to AuthorizeNet, no response received.', 'dti-authorize-pro');
