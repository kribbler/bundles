...
<script>
	jQuery(document).ready(function($) {
		...
		$('#portfolio-grid:not(.loading) li:not(.portfolio_hidden) img[data-isrc]').livequery(function() {
			var i=$(this).data('isrc');
			$(this).attr('src',i);
			$(this).removeAttr('data-isrc');
		});
		$(window).load(function(){
			...
			$('.filter-wrapper li:not(.active):eq(0) a').click()
			$('#portfolio-grid').hide().removeClass('loading').fadeIn();
		});
...
<?php 
	...
	// Originally grabbed an image at specific dimensions.
	// $image = wp_get_attachment_image_src( $thumb,'portfolio');
	$image = wp_get_attachment_image_src( $thumb,'thumbnail');
	$image   = (!empty($image[0])) ? $image[0] : '';
	
	if (empty($image) || empty($img_url))
		continue;
...
?>
	<li class="portfolio_item <?php foreach( $portfolio_cats as $portfolio_cat ) { echo ' term_id_' . $portfolio_cat->term_id; } ?> <?php foreach( $portfolio_tags as $portfolio_tag ) { echo ' term_id_' . $portfolio_tag->term_id; } ?>">
		...
		<?php 
			...
			<figure class="thumbnail thumbnail__portfolio" style="height:175px">
				<a href="<?php echo $link_href; ?>" class="image-wrap" title="<?php echo $link_title; ?>" <?php echo $link_rel; ?>>
					<img data-isrc="<?php echo $image ?>" alt="<?php the_title(); ?>" />
					...
					foreach ( $images as $attachment_id => $attachment ) {
						if ( ($attachment->ID == $thumbid) ) continue;
							...
							if (!$attachment_counter && !has_post_thumbnail()) {
								...
								$img_tag       = '<img data-isrc="'.$image.'" alt="'.$image_title.'" />';
							} else {
								$figure_before = '';
								$figure_after  = '';
								$link_href = $image_attributes[0];
			...
			<div class="caption caption__portfolio">
				<?php if($folio_title == "yes"){ ?>
					<h3><a href="<?php the_permalink(); ?>"><?php $title = the_title('','',FALSE); echo $title;/*substr($title, 0, 40);*/ ?></a></h3>
				<?php } ?>

				<?php if($folio_excerpt == "yes"){ ?>
					<p class="excerpt"><?php $excerpt = get_the_excerpt(); echo $excerpt;/*my_string_limit_words($excerpt,$folio_excerpt_count);*/?></p>
				<?php } ?>
