					...
					if ($thumb == 'true') {
						if (has_post_thumbnail($post->ID) && $format == 'image') {
							...
							if ( $images ) {
								...
								wp_enqueue_style('prettyPhoto','/wp-content/themes/CherryFramework/css/prettyPhoto.css');
								wp_enqueue_script('prettyPhoto','/wp-content/themes/CherryFramework/js/jquery.prettyPhoto.js');
								wp_enqueue_script('prettyPhotoDo','/wp-content/themes/theme48103/jquery.prettyPhoto.do.js');
								$output .= '<figure class="featured-thumbnail">';
								$output .= '<a href="'.$url.'" title="'.get_the_title($post->ID).'" rel="prettyPhoto">';
								$output .= '<img src="'.$image.'" alt="'.get_the_title($post->ID).'" />';
