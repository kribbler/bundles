...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			/* condition start */
			if(!empty($concept7_data) || !empty($data)){
...
"Zeyada	   				"
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					$t_value = '';
					$t_value = stripslashes($concept7_data[$value['id']]);
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {			
						$output .= '<option id="' . $select_ID . '" value="'.$option.'" ' . selected($concept7_data[$value['id']], $option, false) . ' />'.$option.'</option>';	 
					 } 
					$output .= '</select></div>';
				break;
				...
				case 'textarea':	
					...
					if(isset($value['options'])){
						...
						$ta_value = stripslashes($concept7_data[$value['id']]);			
						$output .= '<textarea class="of-input" name="'.$value['id'].'" id="'. $value['id'] .'" cols="'. $cols .'" rows="8">'.$ta_value.'</textarea>';		
				break;
				
				//radiobox option
				case "radio":
					$checked = (isset($concept7_data[$value['id']])) ? checked($concept7_data[$value['id']], $option, false) : '';
					 foreach($value['options'] as $option=>$name) {
						$output .= '<input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($concept7_data[$value['id']], $option, false) . ' /><label class="radio">'.$name.'</label><br/>';				
					}
				break;
				
				//checkbox option
				case 'checkbox':
					if (!isset($concept7_data[$value['id']])) {
						$concept7_data[$value['id']] = 0;
					}
					...
					$output .= '<input type="checkbox" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($concept7_data[$value['id']], 1, false) .' />';
				break;
				
				//multiple checkbox option
				case 'multicheck': 			
					(isset($concept7_data[$value['id']]))? $multi_stored = $concept7_data[$value['id']] : $multi_stored="";
				...
				//colorpicker option
				case 'color':		
					$output .= '<div id="' . $value['id'] . '_picker" class="colorSelector"><div style="background-color: '.$concept7_data[$value['id']].'"></div></div>';
					$output .= '<input class="of-color" name="'.$value['id'].'" id="'. $value['id'] .'" type="text" value="'. $concept7_data[$value['id']] .'" />';
				break;
				
				//typography option	
				case 'typography':
				
					$typography_stored = isset($concept7_data[$value['id']]) ? $concept7_data[$value['id']] : $value['std'];
				...
				case 'tk_typography':
					...
					$tk_typography_stored = isset($concept7_data[$value['id']]) ? $concept7_data[$value['id']] : $value['std'];
					
					/* Font Size */
					
					if(isset($tk_typography_stored['size'])) {
						$output .= '<div class="select_wrapper typography-size" original-title="Font size">';
						$output .= '<select class="of-typography of-typography-size select" name="'.$value['id'].'[size]" id="'. $value['id'].'_size">';
							for ($i = 9; $i < 20; $i++){ 
								$test = $i.'px';
								$output .= '<option value="'. $i .'px" ' . selected($tk_typography_stored['size'], $test, false) . '>'. $i .'px</option>'; 
								}
					...
					/* Line Height */
					if(isset($tk_typography_stored['height'])) {
					
						$output .= '<div class="select_wrapper typography-height" original-title="Line height">';
						$output .= '<select class="of-typography of-typography-height select" name="'.$value['id'].'[height]" id="'. $value['id'].'_height">';
							for ($i = 20; $i < 38; $i++){ 
								$test = $i.'px';
								$output .= '<option value="'. $i .'px" ' . selected($tk_typography_stored['height'], $test, false) . '>'. $i .'px</option>'; 
								}
					...
					/* Font Face */
					if(isset($tk_typography_stored['face'])) {
					
						$mini ='';
						if(!isset($value['mod'])) $value['mod'] = '';
						if($value['mod'] == 'mini') { $mini = 'mini';}
						
						$output .= '<input style="width:150px; float:left; margin-right:5px;" class="of-input '.$mini.'" name="'.$value['id'].'[face]" id="'. $value['id'] .'_face" type="text" value="'. $tk_typography_stored['face'] .'" />';
					
					}
					
					/* Font Weight */
					if(isset($tk_typography_stored['style'])) {
						...
						foreach ($styles as $i=>$style){
							...
							$output .= '<option value="'. $i .'" ' . selected($tk_typography_stored['style'], $i, false) . '>'. $style .'</option>';		
						}
						$output .= '</select></div>';
					...
					/* Font Color */
					if(isset($tk_typography_stored['color'])) {
					
						$output .= '<div id="' . $value['id'] . '_color_picker" class="colorSelector typography-color"><div style="background-color: '.$tk_typography_stored['color'].'"></div></div>';
						$output .= '<input class="of-color of-typography of-typography-color" original-title="Font color" name="'.$value['id'].'[color]" id="'. $value['id'] .'_color" type="text" value="'. $tk_typography_stored['color'] .'" />';
				...
				case 'font':
					...
					$font_stored = isset($concept7_data[$value['id']]) ? $concept7_data[$value['id']] : $value['std'];
										
					/*Font */
					$output .= '<div class="select_wrapper border-style" style="width:200px;">';
					$output .= '<select class="of-border of-border-style select" name="'.$value['id'].'[font]" id="'. $value['id'].'_font">';
					
					foreach ($fonts as $font){
						$output .= '<option value="'. $font .'" ' . selected($font_stored['font'], $font, false) . '>'. $font .'</option>';		
					}
					
					$output .= '</select></div>';
					
					/* Font Color */		
					$output .= '<div id="' . $value['id'] . '_color_picker" class="colorSelector"><div style="background-color: '.$font_stored['color'].'"></div></div>';
					$output .= '<input class="of-color of-border of-border-color" name="'.$value['id'].'[color]" id="'. $value['id'] .'_color" type="text" value="'. $font_stored['color'] .'" />';
					
					/* Font Weight */
					
					$output .= '<div class="select_wrapper typography-style" original-title="Font weight" style="float:left; margin-left:5px;">';
					$output .= '<select class="of-typography of-typography-style select" name="'.$value['id'].'[style]" id="'. $value['id'].'_style">';
					$styles = array('100',
									'200',
									'300',
					...
					foreach ($styles as $style){
						...
						$output .= '<option value="'. $style .'" ' . selected($font_stored['style'], $style, false) . '>'. $style .'</option>';		
					}
					$output .= '</select></div>';
				...
				case 'typekit':
					...
					$typekit_stored = isset($concept7_data[$value['id']]) ? $concept7_data[$value['id']] : $value['std'];
										
					$mini ='';
					if(!isset($value['mod'])) $value['mod'] = '';
					if($value['mod'] == 'mini') { $mini = 'mini';}
					
					if(isset($typekit_stored['font'])) {
					$output .= '<input style="width:200px; float:left; margin-right:5px;" class="of-input '.$mini.'" name="'.$value['id'].'[font]" id="'. $value['id'] .'_font" type="text" value="'. $typekit_stored['font'] .'" />';
					}
					
					/* Border Color */	
					if(isset($typekit_stored['color'])) {	
					$output .= '<div id="' . $value['id'] . '_color_picker" class="colorSelector"><div style="background-color: '.$typekit_stored['color'].'"></div></div>';
					$output .= '<input class="of-color of-border of-border-color" name="'.$value['id'].'[color]" id="'. $value['id'] .'_color" type="text" value="'. $typekit_stored['color'] .'" />';
					}
				...
				case 'border':
					...
					/* Border Width */
					$border_stored = $concept7_data[$value['id']];
				...
				case 'images':
					...
					$select_value = (isset($concept7_data[$value['id']])) ? $concept7_data[$value['id']] : '';
				...
				case 'sorter':
					...
					$sortlists = isset($concept7_data[$value['id']]) && !empty($concept7_data[$value['id']]) ? $concept7_data[$value['id']] : $value['std'];
				...
				case 'tiles':
					...
					$i = 0;
					$select_value = isset($concept7_data[$value['id']]) && !empty($concept7_data[$value['id']]) ? $concept7_data[$value['id']] : '';
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ', 'concept7').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
				...
				case 'transfer':
					...
					$instructions = $value['desc'];
					$output .= '<textarea id="export_data" rows="8">'.base64_encode(serialize($concept7_data)) /* 100% safe - ignore theme check nag */ .'</textarea>'."\n";
				...
				case 'select_google_font':
					...
					foreach ($value['options'] as $select_key => $option) {
						$output .= '<option value="'.$select_key.'" ' . selected((isset($concept7_data[$value['id']]))? $concept7_data[$value['id']] : "", $option, false) . ' />'.$option.'</option>';
					} 
					$output .= '</select></div>';
				...
				case 'sliderui':
					...
					$s_val  = stripslashes($concept7_data[$value['id']]);
				...
				case 'switch':
					...
					$output .= '<p class="switch-options">';
						...
						$output .= '<input type="hidden" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="0"/>';
						$output .= '<input type="checkbox" id="'.$value['id'].'" class="'.$fold.'checkbox of-input main_checkbox" name="'.$value['id'].'"  value="1" '. checked($concept7_data[$value['id']], 1, false) .' />';
	...
	public static function optionsframework_sidebar_function($id,$std,$oldorder,$order,$int){
		...
		$concept7_data = get_option(OPTIONS);
