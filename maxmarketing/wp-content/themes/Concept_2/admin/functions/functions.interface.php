...
function of_ajax_callback() 
{
	...
	elseif($save_type == 'restore_options')
	{
		...
		$concept7_data = get_option(BACKUPS);

		update_option(OPTIONS, $concept7_data);

		of_save_options($concept7_data);
	...
	elseif($save_type == 'import_options'){
		...
		$concept7_data = unserialize(base64_decode($concept7_data)); //100% safe - ignore theme check nag
		of_save_options($concept7_data);
	...
	elseif ($save_type == 'save')
	{
		...
		wp_parse_str(stripslashes($_POST['data']), $concept7_data);
		unset($concept7_data['security']);
		unset($concept7_data['of_save']);
		of_save_options($concept7_data);
